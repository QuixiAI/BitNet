...
// ---------------------------------------------------------------------------
// BACKWARD grouped GEMMs (built on request past moe_train_plan §0.3's "training
// never depends on custom kernels" — the fused-Metal training path for the
// truncated smoke model / A-track MoE experiments). Same segmented
// one-expert-per-32-row-tile structure as the forwards; DENSE weights (the STE
// backward is dense by construction, train_plan §4).
//
//   bwd_dx: dX(rows, K_dim) = dY(rows, N_out) @ W[e]^T          (mma_ABt; W in its
//           native (E, K_dim, N_out) layout — no transpose copy)
//   bwd_dw: dW[e](K_dim, N_out) = A^T @ dY over expert e's padded segment
//           (mma_AtB; one threadgroup owns one (e, 32x32) output tile, so no
//           atomics). moe_gather zero-fills pad rows of A and the host zero-fills
//           pad rows of dY (moe_finalize_bwd writes only real positions), so the
...
kernel void moe_grouped_gemm_bwd_dx(device T *dx                     [[buffer(0)]],
                                    ...
                                    uint  simd_lane_id   [[thread_index_in_simdgroup]]) {
    const int OY = (int)threadgroup_id.y;    // row-tile
    const int OX = (int)threadgroup_id.x;    // output column-tile (in K_dim)
    const int e = expert_of_tile[OY];
    if (e < 0) { return; }
    ...
    rt<T, N_BE, K_BE> a_reg;                              // dY tile (32 rows x 16 of N_out)
    rt<T, M_BE, K_BE, ducks::rt_layout::col> b_reg;       // W tile (32 of K_dim x 16 of N_out)
    rt<float, N_BE, M_BE> d_reg;
    zero(d_reg);
...
template <typename T, unsigned N_BLOCK, unsigned K_BLOCK, unsigned M_BLOCK>
kernel void moe_grouped_gemm_bwd_dw(device T *dw                     [[buffer(0)]],  // (E, K_dim, N_out)
                                    device T *A                      [[buffer(1)]],  // (rows, K_dim)
                                    device T *dy                     [[buffer(2)]],  // (rows, N_out)
                                    device const int *off_pad        [[buffer(3)]],  // (E+1)
                                    constant int &total_rows         [[buffer(4)]],
                                    constant int &K_dim              [[buffer(5)]],
                                    constant int &N_out              [[buffer(6)]],
                                    uint3 threadgroup_id [[threadgroup_position_in_grid]],
                                    uint  simd_lane_id   [[thread_index_in_simdgroup]]) {
    const int NX = (int)threadgroup_id.x;    // output column-tile (in N_out)
    const int KY = (int)threadgroup_id.y;    // output row-tile (in K_dim)
    const int e  = (int)threadgroup_id.z;
    const int start = off_pad[e], end = off_pad[e + 1];   // 32-multiples
    ...
    rt<T, K_BE, N_BE, ducks::rt_layout::col> a_reg;       // A tile (16 token rows x 32 of K_dim)
    rt<T, K_BE, M_BE> b_reg;                              // dY tile (16 token rows x 32 of N_out)
    rt<float, N_BE, M_BE> d_reg;
    zero(d_reg);
    ...
    store(gl_d, d_reg, {0, 0, KY, NX}, simd_lane_id);    // empty segment -> zeros
}
...
// ---------------------------------------------------------------------------
// Backward of moe_finalize: grad_expert_out[pos] = w · grad_out[token] (host
// zero-fills grad_expert_out so pad rows stay exactly zero) and the router-weight
// grad grad_w[t,k] = <expert_out[pos], grad_out[token]>. Each pos belongs to one
// (token, k) — no atomics. One simdgroup per token.
// ---------------------------------------------------------------------------
template <typename T>
kernel void moe_finalize_bwd(device const T     *grad_out     [[buffer(0)]],  // (T, Hdim)
                             device const T     *expert_out   [[buffer(1)]],  // (P, Hdim)
                             device const int   *inv_idx      [[buffer(2)]],  // (T*K)
                             device T           *grad_eo      [[buffer(3)]],  // (P, Hdim) pre-zeroed
                             device float       *grad_w       [[buffer(4)]],  // (T, K)
                             device const float *topk_weights [[buffer(5)]],  // (T, K)
...
// Backward of moe_gather: dx[token] = Σ_k dA[inv_pad[token·K + k]] (each token
// owns its K contributions — the scatter-add without atomics).
template <typename T>
kernel void moe_gather_bwd(device const T   *dA      [[buffer(0)]],   // (P, Hdim)
                           device const int *inv_idx [[buffer(1)]],   // (T*K)
                           device T         *dx      [[buffer(2)]],   // (T, Hdim)
...
instantiate_moe_gemm_q(bitnet, bitnet)   // BitNet ternary experts (bitnet_train)
