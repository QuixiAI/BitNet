...
// ---- W2A8 v2: one WHOLE block per lane. Each lane owns 10-byte block g (lane-consecutive
// blocks -> the simdgroup streams 320 contiguous weight bytes/iteration), unpacks each code
// byte into 4 int8 lanes arithmetically (spread = (b | b<<6 | b<<12 | b<<18) & 0x03030303),
// and accumulates via idot4 against two uint4 X loads (32 int8, 32B-aligned). The {0,1,2}
// code bias is removed once per block (isum - sum(x)), and the group scale multiplies the
// integer subtotal once per block — 32 codes per scale multiply vs v1's 8. ----
kernel void qgemv_w2a8_v2(
    ...
    for (int g = (int)lane; g < bpr; g += 32) {
        ...
        int isum = 0, xsum = 0;
        #pragma clang loop unroll(full)
        for (int j = 0; j < 4; ++j) {                     // qs bytes 2j, 2j+1 -> 8 codes
            const uint b2 = (uint)((device const ushort*)(base + 2))[j];   // 2-aligned
        ...
        lane_acc += float(isum - xsum) * float(gscale);   // codes {0,1,2} -> {-1,0,+1}
    }
    const float facc = metal::simd_sum(lane_acc);
    if (lane == 0) D[row] = half(facc * float(a_scale[0]));
}
