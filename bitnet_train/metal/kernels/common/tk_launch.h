...
//        D in {256,512,768,1024} uses the register-resident template; other D%4==0 widths
//        route to the streaming dyn kernel (D@5) — full rows at 2048+ would blow registers. -----
template <class E>
void launch_rms_norm(E& e, typename E::in_t x, typename E::in_t w,
                     typename E::out_t o, uint32_t M, int D, float eps) {
  const bool templated = (D == 256 || D == 512 || D == 768 || D == 1024);
  e.pipeline(templated ? rms_norm_kernel_name(D) : "mittens::rms_norm_dyn");
  e.in(x, 0); e.in(w, 1); e.out(o, 2);
  e.bytes(M, 3); e.bytes(eps, 4);
  if (!templated) e.bytes(D, 5);
  e.dispatch(static_cast<int>(M), 1, 1, 32, 1, 1);
}
template <class E>
...
// ----- weight_quant_ternary (K1/K5, BitNet training): W@0(E,N,K) -> wq@1(u8 bitnet blocks)
//        w_deq@2(bf16) ; K@3 group_k@4 N@5 (i32) ; grid (N,E,1), 32 thr. Per-group absmean;
//        batched over the expert axis (E=1 is the plain 2-D case). -----
template <class E>
...
// ----- gemm_v3 (ACADEMIC): D@0 A@1 B@2 ; N@3 K@4 M@5 ; grid (M/64, N/64, 1), 128 thr
//        (2x2 warps over a 64x64 block, both operands staged). N%64, M%64, K%32. -----
template <class E>
...
// ----- qgemm_bwd (ACADEMIC, bitnet): D@0(M,K half) G@1(M,N half) Wq@2(packed) ; M@3 N@4 K@5 ;
//        grid (K/32, M/32, 1), 64 thr (2 warps split M, sharing the dequantized W tile). -----
template <class E>
...
// ----- kd_kl_topk (sparse-teacher KD-KL): fwd logits@0 t_idx@1(i32 Tn,K) t_prob@2(f32) ->
//        loss@3 lse@4 ; V@5 K@6 invtemp@7(f32) tail_mode@8 ; grid (Tn,1,1), 32 thr.
//        bwd adds lse@3 grad_out@4 -> grad_logits@5. tail_mode: 0 renorm, 1 other-bucket. -----
template <class E>
...
// ----- fake_quant_int8 (K4): x@0 -> x_q@1(bf16) codes@2(i8) scale@3(f32) ; D@4 ;
//        grid (rows,1,1), 32 thr. One-pass per-token int8 FAKE-quant (x_q on the
//        half-rounded grid). silu_mul variant adds gate@1, mode/alpha/limit. -----
template <class E>
...
// ----- weight_quant_ternary_pt (per-TENSOR absmean, two passes): -----
// pass 1: W@0 abssum@1(atomic f32[E], host-zeroed) NK@2 ; grid (ceil(NK/4096), E), 256 thr.
template <class E>
...
// pass 2: W@0 abssum@1(f32[E]) wq@2 w_deq@3 ; K@4 N@5 ; grid (N, E), 32 thr.
template <class E>
...
//        stats: wq@0 -> counts@1(u32 rows,3 = [-1,0,+1]) ; nblocks@2 ; grid (rows,1,1), 32 thr.
//        flips: wq_a@0 wq_b@1 -> flips@2(u32 rows) ; nblocks@3 ; same grid. -----
template <class E>
...
// ----- fake_quant_fp8 (mode b / Q-T4 e4m3 fake-quant), pass 2 after
//        launch_quant_tensor_absmax: x@0 scale_u@1 -> x_fq@2 scale_out@3 ; n@4 ;
//        vec4 per thread. -----
template <class E>
...
// ----- kd_kl_dense (A6b full-KL, live teacher logits): fwd t@0 s@1 -> loss@2 lse_t@3
//        lse_s@4 ; V@5 invtemp@6 ; grid (Tn,1,1), 32 thr. bwd: t@0 s@1 lse_t@2 lse_s@3
//        grad_out@4 -> grad_s@5 ; V@6 invtemp@7. -----
template <class E>
...
// ----- attn_decode (batch-1 GQA decode, dense KV): q@0(Hq,D) kc@1 vc@2(Tk,Hkv,D) ->
//        out@3(Hq,D) ; Tk@4 Hq@5 Hkv@6 D@7 ; grid (Hq,1,1), 32 thr, D <= 128. -----
template <class E>
...
// bwd_dx: dx@0(rows,K_dim) dy@1(rows,N_out) W@2(E,K_dim,N_out) eot@3 ; rows@4 K_dim@5
//         N_out@6 ; grid (K_dim/32, rows/32), 32 thr.
template <class E>
void launch_moe_grouped_gemm_bwd_dx(E& e, typename E::out_t dx, typename E::in_t dy,
                                    typename E::in_t w, typename E::in_t expert_of_tile,
...
// bwd_dw: dw@0(E,K_dim,N_out) A@1(rows,K_dim) dy@2(rows,N_out) off_pad@3(E+1) ; rows@4
//         K_dim@5 N_out@6 ; grid (N_out/32, K_dim/32, E), 32 thr.
template <class E>
void launch_moe_grouped_gemm_bwd_dw(E& e, typename E::out_t dw, typename E::in_t a,
                                    typename E::in_t dy, typename E::in_t off_pad,
...
// finalize_bwd: grad_out@0(T,H) expert_out@1(P,H) inv_idx@2(T*K) -> grad_eo@3(P,H,
//         host-zeroed) grad_w@4(f32 T,K) ; weights@5 K@6 H@7 ; grid (T,1,1), 32 thr.
template <class E>
void launch_moe_finalize_bwd(E& e, typename E::in_t grad_out, typename E::in_t expert_out,
                             typename E::in_t inv_idx, typename E::out_t grad_eo,
                             typename E::out_t grad_w, typename E::in_t topk_weights,
...
// gather_bwd: dA@0(P,H) inv_idx@1(T*K) -> dx@2(T,H) ; K@3 H@4 ; grid (T,1,1), 32 thr.
template <class E>
void launch_moe_gather_bwd(E& e, typename E::in_t dA, typename E::in_t inv_idx,
                           typename E::out_t dx, int T, int K, int Hdim,
                           const std::string& type_name) {
...
// ----- adamw_masked (cold-expert decay mask, moe_train_plan §4.3): adamw buffers +
//        mask@15(u8[S]) seg_size@16(u32) mask_mode@17(i32; 0 skip-update, 1 skip-decay). -----
template <class E>
...
void launch_qgemv_w2a8(E& e, typename E::out_t d, typename E::in_t wq, typename E::in_t xq,
                       typename E::in_t ascale, int N, int K, int version = 1) {
  // v2: one whole 10-byte block per lane, arithmetic 2-bit->int8 spread + idot4, one scale
  // multiply per 32 codes. v1: 4 lanes/block, scalar unpack. Route by measurement.
  e.pipeline(version == 2 ? "mittens::qgemv_w2a8_v2" : "mittens::qgemv_w2a8");
