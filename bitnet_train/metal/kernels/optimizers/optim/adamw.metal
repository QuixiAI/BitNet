...
// ---------------------------------------------------------------------------
// Cold-expert MASKED AdamW (moe_train_plan §4.3/§5.6): decoupled decay fires every
// step whether or not a parameter received gradient, so a rarely-routed expert's
// latents shrink monotonically toward the zero-code region — decay manufactures
// dead ternary experts. Element gid belongs to segment gid/seg_size (one segment
// per expert slice); mask[seg]==0 means "expert received no tokens this step".
//   mask_mode 0: masked segments SKIP THE ENTIRE UPDATE (params/moments pass
//                through untouched — the plan's CI-testable semantics).
//   mask_mode 1: masked segments skip ONLY the decay term (moments still decay
//                toward zero on the zero gradient — the softer variant).
// ---------------------------------------------------------------------------
...
kernel void adamw_masked(device const T     *param     [[buffer(0)]],
    ...
    if (!active && mask_mode == 0) {              // cold expert: untouched this step
    ...
    const float vhat = v / bc2;
    const float p = float(param[gid]);
    const float decay = active ? wd : 0.0f;       // mode 1: cold segments skip decay only
