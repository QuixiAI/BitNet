...
// ---------------------------------------------------------------------------
// K4 — fused per-token int8 FAKE-quant (BitNet training forward).
//
// The training BitLinear needs the fake-quant activation x_q = codes * half(scale)
// as a bf16 tensor (it feeds both the dense fake-quant GEMM and the STE W-grad).
// Composing quantize_per_token_int8 + eager dequant costs 3 extra passes over
// (M, K) in fp32 (measured 343 µs vs the 182 µs quantizer at 2048²); these
// kernels emit x_q, codes, and scale in ONE pass.
//
//   scale s = amax/127 (f32 out);  codes = round-clamp(x/s) in [-127,127];
//   x_q = bf16(codes * half(s))   — the SAME half-rounded grid the integer
//   kernels (qgemm_w2a8/qgemv_w2a8) consume, so train/infer numerics agree.
//
// silu_mul_fake_quant_int8 is the FFN epilogue analogue (buffer order matches
// tk.glu / silu_mul_quant_int8: x = activated operand, gate = multiplier;
// mode 0 = swiglu, 1 = swiglu_oai via the shared glu_eval).
// ---------------------------------------------------------------------------
...
kernel void fake_quant_int8(device const T *x     [[buffer(0)]],
    ...
    const int nchunks = D / 4;                       // binding asserts D % 4 == 0
    float amax = 0.0f;
    ...
    const float sh  = float(half(s));                // the half-rounded dequant grid
    for (int c = (int)lane; c < nchunks; c += 32) {
...
kernel void silu_mul_fake_quant_int8(device const T *x     [[buffer(0)]],
    ...
    const int gmode = (mode == 1) ? 3 : 2;           // glu_eval: 2 swiglu, 3 swiglu_oai
    float amax = 0.0f;
    for (int c = (int)lane; c < nchunks; c += 32) {
