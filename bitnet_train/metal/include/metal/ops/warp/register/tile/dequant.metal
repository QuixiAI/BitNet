...
// ---- tq2_0 : llama.cpp/ggml native ternary (the TQ2_0 GGUF type — the plans' upstream export
//   route). 256-element block, per-block ABSMAX half scale, 2-bit codes in {0,1,2} -> value
//   d*(code-1). ggml layout { uint8 qs[64]; half d; } = 66 bytes — scale LAST, and the code order
//   interleaves: element 128j + 32n + m lives in qs[32j + m] at bits 2n (ggml-quants.c
//   quantize_row_tq2_0_ref, read 2026-07-07). Lets qgemm/qgemv/moe_grouped_gemm_*_q and qdequant
//   consume llama-quantize output directly — no external runtime needed for TQ2_0 eval. ----
struct tq2_0 {
    constant static constexpr const int block_k     = 256;
    constant static constexpr const int block_bytes = 66;
    static METAL_FUNC half dequant(device const uchar* base, int col) {
