...
// Cold-expert masked AdamW (moe_train_plan §4.3): element i belongs to segment i/seg_size
// (one segment per expert slice); mask[seg]==0 -> skip the whole update (mask_mode 0) or
// just the decay term (mask_mode 1). Returns (param', m', v').
static std::tuple<at::Tensor, at::Tensor, at::Tensor> adamw_masked_mps(
  ...
  TORCH_CHECK(mask_mode == 0 || mask_mode == 1, "adamw_masked: mask_mode in {0,1}");
  auto param = param_in.contiguous();
  auto grad = grad_in.to(param.scalar_type()).contiguous();
...
// K1/K5 (BitNet training): latent weight -> packed bitnet ternary blocks + dequantized bf16
// weight in one pass (same absmean scale for both => STE fwd/bwd consistency). W is (N, K)
// or a batched expert stack (E, N, K). Returns (wq uint8 (..., K/32, 10), w_deq bf16 like W).
static std::tuple<at::Tensor, at::Tensor, int, int, int, at::Tensor>
weight_quant_prep(const at::Tensor& w_in, const char* name) {
  TORCH_CHECK(w_in.device().is_mps(), name, ": W must be an MPS tensor");
  TORCH_CHECK(tk_is_float_dtype(w_in), name, ": W must be float32/float16/bfloat16");
  ...
  qshape.push_back(K / 32); qshape.push_back(10);
...
static std::tuple<at::Tensor, at::Tensor> weight_quant_ternary_mps(const at::Tensor& w_in,
                                                                   int64_t group_k) {
...
// Per-TENSOR absmean (train_plan §4 / moe_train_plan §3.7 baseline): one scale over each
// (N, K) slice, replicated into every packed block. Two passes (abssum reduce, then encode).
...
// ACADEMIC dense GEMM attempt (2x2-warp 64x64 tile, both operands staged): D = A @ B.
...
// ACADEMIC ternary backward GEMM: grad_x (M,K) = grad_y (M,N) @ dequant(Wq) — the kernel
// train_plan §4 says not to build, built to measure the argument. half in/out.
static at::Tensor qgemm_bwd_mps(const at::Tensor& g_in, const at::Tensor& wq_in) {
  ...
  auto g = g_in.contiguous(), wq = wq_in.contiguous();
  const int M = g.size(0), N = wq.size(0);
  const long row_bytes = wq.numel() / N;
  const int K = static_cast<int>(row_bytes / 10 * 32);   // bitnet: 10 bytes / 32 weights
...
// Sparse-teacher KD-KL forward: student logits (Tn,V) + teacher top-k cache
// (t_idx int32 (Tn,K), t_prob f32 (Tn,K)) -> (loss (Tn,), lse (Tn,)).
static std::tuple<at::Tensor, at::Tensor> kd_kl_topk_fwd_mps(
  ...
  TORCH_CHECK(tail_mode == 0 || tail_mode == 1, "kd_kl_topk_fwd: tail_mode in {0,1}");
  auto logits = logits_in.contiguous(); auto t_idx = t_idx_in.contiguous();
...
static at::Tensor kd_kl_topk_bwd_mps(const at::Tensor& logits_in, const at::Tensor& t_idx_in,
                                     const at::Tensor& t_prob_in, const at::Tensor& lse_in,
                                     const at::Tensor& grad_out_in, double invtemp,
                                     int64_t tail_mode) {
  auto logits = logits_in.contiguous(); auto t_idx = t_idx_in.contiguous();
  auto t_prob = t_prob_in.contiguous(); auto lse = lse_in.contiguous();
...
// K4: one-pass per-token int8 fake-quant. Returns (x_q bf16, codes int8, scale f32).
static std::tuple<at::Tensor, at::Tensor, at::Tensor> fake_quant_int8_mps(const at::Tensor& x_in) {
  ...
  auto x = x_in.contiguous();
  const int D = x.size(-1);
  TORCH_CHECK(D % 4 == 0, "fake_quant_int8: D must be a multiple of 4");
  const int rows = static_cast<int>(x.numel() / D);
...
static std::tuple<at::Tensor, at::Tensor, at::Tensor> silu_mul_fake_quant_int8_mps(
  ...
  auto x = x_in.contiguous(), gate = gate_in.contiguous();
  const int D = x.size(-1);
  TORCH_CHECK(D % 4 == 0, "silu_mul_fake_quant_int8: D must be a multiple of 4");
  const int rows = static_cast<int>(x.numel() / D);
  auto x_q = at::empty(x.sizes(), x.options().dtype(at::kBFloat16));
  auto codes = at::empty(x.sizes(), x.options().dtype(at::kChar));
  std::vector<int64_t> sshape(x.sizes().begin(), x.sizes().end() - 1);
...
static at::Tensor ternary_stats_mps(const at::Tensor& wq_in) {
  ...
  const int rows = static_cast<int>(wq.numel() / ((int64_t)nblocks * 10));
...
static at::Tensor code_flip_count_mps(const at::Tensor& a_in, const at::Tensor& b_in) {
  ...
  const int rows = static_cast<int>(a.numel() / ((int64_t)nblocks * 10));
...
// Per-tensor e4m3 FAKE-quant (mode b / Q-T4). Returns (x_fq same dtype, scale f32 scalar).
...
// Dense-teacher KD-KL (A6b): fwd -> (loss, lse_t, lse_s); bwd -> grad_s.
static std::tuple<at::Tensor, at::Tensor, at::Tensor> kd_kl_dense_fwd_mps(
  ...
  auto t = t_in.contiguous(), s = s_in.contiguous();
...
static at::Tensor kd_kl_dense_bwd_mps(const at::Tensor& t_in, const at::Tensor& s_in,
                                      ...
                                      const at::Tensor& grad_out_in, double invtemp) {
  auto t = t_in.contiguous(), s = s_in.contiguous();
  auto lse_t = lse_t_in.contiguous(), lse_s = lse_s_in.contiguous();
  auto grad_out = grad_out_in.to(at::kFloat).contiguous();
  const int rows = t.size(0), V = t.size(1);
...
static at::Tensor qgemm_w2a8_fused_mps(const at::Tensor& wq_in, const at::Tensor& x_in) {
  ...
  TORCH_CHECK(wq_in.scalar_type() == at::kByte && wq_in.dim() >= 2, "qgemm_w2a8_fused: wq uint8");
  auto x = x_in.contiguous(), wq = wq_in.contiguous();
...
// Batch-1 GQA attention decode against a dense KV cache. q (Hq,D), kc/vc (Tk,Hkv,D).
...
// MoE backward grouped GEMMs + glue.
static at::Tensor moe_grouped_gemm_bwd_dx_mps(const at::Tensor& dy_in, const at::Tensor& W_in,
  ...
  TORCH_CHECK(dy_in.dim() == 2 && W_in.dim() == 3, "moe_grouped_gemm_bwd_dx: dy (rows,N), W (E,K,N)");
  ...
  auto dy = dy_in.contiguous(), W = W_in.contiguous();
  auto eot = eot_in.to(at::kInt).contiguous();
...
static at::Tensor moe_grouped_gemm_bwd_dw_mps(const at::Tensor& A_in, const at::Tensor& dy_in,
  ...
  auto A = A_in.contiguous(), dy = dy_in.contiguous();
...
// Backward of moe_finalize: -> (grad_expert_out (P,H) zero-padded, grad_weights f32 (T,K)).
static std::tuple<at::Tensor, at::Tensor> moe_finalize_bwd_mps(
  ...
  TORCH_CHECK(inv_in.numel() == (int64_t)T * K, "moe_finalize_bwd: inv_pad must be (T*K,)");
  auto grad_out = grad_out_in.contiguous(), expert_out = expert_out_in.contiguous();
  auto inv = inv_in.to(at::kInt).contiguous();
  auto weights = weights_in.to(at::kFloat).contiguous();
  auto grad_eo = at::zeros(expert_out.sizes(), expert_out.options());   // pads stay zero
...
// Backward of moe_gather: dx[t] = sum_k dA[inv_pad[t*K+k]].
...
static at::Tensor qgemv_w2a8_mps(const at::Tensor& wq_in, const at::Tensor& xq_in,
  ...
  tk_encode([&](TorchEncoder& e) {
    tk::launch_qgemv_w2a8(e, out, wq, xq, as, N, K, static_cast<int>(version));
  });
  return out;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("adamw_masked", &adamw_masked_mps,
        "Cold-expert masked AdamW step: mask u8[S] per seg_size-element segment; "
        "mask_mode 0 = skip update, 1 = skip decay only (MPS)");
  ...
  m.def("weight_quant_ternary", &weight_quant_ternary_mps,
        "BitNet K1/K5: latent weight (N,K)|(E,N,K) -> (packed ternary blocks, dequantized bf16), per-group absmean (MPS)",
        pybind11::arg("w"), pybind11::arg("group_k") = 32);
  m.def("gemm_v3", &gemm_v3_mps, "ACADEMIC 2x2-warp staged GEMM attempt vs MPS (MPS)");
  m.def("qgemm_bwd", &qgemm_bwd_mps,
        "ACADEMIC ternary backward GEMM: grad_y @ dequant(bitnet blocks) (MPS)");
  m.def("kd_kl_topk_fwd", &kd_kl_topk_fwd_mps,
        "Sparse-teacher KD-KL forward -> (loss, lse); tail_mode 0 renorm / 1 other-bucket (MPS)",
        pybind11::arg("logits"), pybind11::arg("t_idx"), pybind11::arg("t_prob"),
        pybind11::arg("invtemp") = 1.0, pybind11::arg("tail_mode") = 0);
  m.def("kd_kl_topk_bwd", &kd_kl_topk_bwd_mps,
        "Sparse-teacher KD-KL backward -> grad_logits (MPS)",
        pybind11::arg("logits"), pybind11::arg("t_idx"), pybind11::arg("t_prob"),
        pybind11::arg("lse"), pybind11::arg("grad_out"),
        pybind11::arg("invtemp") = 1.0, pybind11::arg("tail_mode") = 0);
  m.def("fake_quant_int8", &fake_quant_int8_mps,
        "BitNet K4: one-pass per-token int8 fake-quant -> (x_q bf16, codes, scale) (MPS)");
  m.def("silu_mul_fake_quant_int8", &silu_mul_fake_quant_int8_mps,
        "BitNet K4: fused SwiGLU + per-token int8 fake-quant (MPS)",
  ...
  m.def("fake_quant_fp8", &fake_quant_fp8_mps,
        "Per-tensor e4m3 fake-quant (mode b / Q-T4) -> (x_fq, scale) (MPS)");
  m.def("kd_kl_dense_fwd", &kd_kl_dense_fwd_mps,
        "Dense-teacher KD-KL forward (A6b) -> (loss, lse_t, lse_s) (MPS)",
  ...
  m.def("attn_decode", &attn_decode_mps,
        "Batch-1 GQA attention decode vs dense KV cache: q (Hq,D) x (Tk,Hkv,D) (MPS)");
